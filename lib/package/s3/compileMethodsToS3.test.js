...
const template = {
  ...
  Properties: {
    ...
    MethodResponses: [
      ...
      { ResponseParameters: {}, ResponseModels: {}, StatusCode: 206 }
    ]
  }
}
...
describe('#compileMethodsToS3()', () => {
  ...
  const testSingleProxy = (opts) => {
    ...
    const methodResponse2 = resource.Properties.MethodResponses.find((x) => x.StatusCode === 206)
    methodResponse2.ResponseParameters = responseParams
  ...
  const testAuthorization = (auth) => {
    ...
    const requestParams = { 'method.request.path.key': true, 'method.request.header.Range': true }
